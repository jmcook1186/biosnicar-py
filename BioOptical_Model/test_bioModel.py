...
def bioptical_calculations(MAC_calculated, MAC_path, biovolume, density, xw, cell_volume, wvl, packaging_correction, pigment_dir, pigments_data, n_algae, k_water,
                            smooth, window_size, poly_order, smoothStart, smoothStop, plot_optical_props, 
                            plot_pigment_MACs, savefiles, savefilename, saveplots_MAC_k, savepath):
    ...
    ## MAC calculation
    ##################      
    # open mass absorption coefficients (m2/kg to m2/mg) for each algal pigment

    # for file in os.listdir(pigment_dir): 
    #     if ("csv") in file:
    #         pigm=(np.array(pd.read_csv(str(pigment_dir+file)))).flatten() # m2/mg_pigm
    #         print(np.size(pigm))
    #         abs_cff_pigments[file]=pigm 
    #         conc = float(input('Enter concentration of  pigment ' + file + '  (mg_pigm/µm3 for biovolume, mg pigm/cell for dry mode)    ->    '))
    #         abs_coeff = abs_coeff + conc*pigm[::10] # in m2/µm3 cells or m2/mg cell rescaled to snicar resolution
    ...
    if MAC_calculated: 
        for key,value in pigments_data.items():
            ...
            conc = value #mg/µm3 or ng/cell
            abs_coeff = abs_coeff + conc*abs_pigm # in m2/µm3 cells or 10e6 m2/cell
        if biovolume:
            MAC = abs_coeff/density # from m2/µm3 to m2/kg wet biomass, dividing by density in kg/µm3
        else:
            MAC = abs_coeff/1000000 # m2/cell  
    
    else:
        MAC = (np.array(pd.read_csv(MAC_path))).flatten()
    ...
    if biovolume:
        k = MAC * density * wvl / 4 / np.pi * 10**(12) # units: abs coeff (converted from m2/µm3 to µm2/µm3 so *10**12) * wvl (µm)
        n=n_algae
    else:         
        k = (xw * k_water) + ((1 - xw) * (wvl/np.pi*4) / cell_volume * MAC * 10**(12)) # units: MAC (converted from m2/cell to µm2/cell) / volume cell (µm3) * wvl (µm)
        n=n_algae
    
    ###############################
    ## MAC, k storage and rescaling
    ###############################
    
    wvl_rescaled_BioSNICAR = wvl[::10]
    MAC_rescaled_BioSNICAR = MAC[::10]
    k_rescaled_BioSNICAR = k[::10]
    n_rescaled_BioSNICAR = n[::10]

    data['wvl'] = wvl_rescaled_BioSNICAR
    data['MAC'] = MAC_rescaled_BioSNICAR #in m2/kg dry or wet biomass
    ...
    if plot_optical_props:
        ...
        plt.subplot(2,1,1)
        plt.plot(wvl[100:700]*1000,MAC[100:700]*10**10)
        plt.xticks(fontsize=24), plt.yticks(fontsize=24)
        plt.xlabel('Wavelength',fontsize=24),plt.ylabel(str('MAC ($10^{-10}$ m$^2$ cell$^{-1}$)'),fontsize=24)
        plt.tight_layout()
    
        plt.subplot(2,1,2)
        plt.plot(wvl[100:700]*1000,k[100:700]*1000)
        plt.xticks(fontsize=24), plt.yticks(fontsize=24)
        plt.xlabel('Wavelength',fontsize=24),plt.ylabel('k ($10^{-3}$)',fontsize=24)
        plt.tight_layout()
    ...
    return wvl, wvl_rescaled_BioSNICAR, k, k_rescaled_BioSNICAR, n, n_rescaled_BioSNICAR, MAC, MAC_rescaled_BioSNICAR, data, abs_cff_pigments
...
def net_cdf_updater(savepath, filename, wvl, g, ssa, MAC, L, r, density, information):
