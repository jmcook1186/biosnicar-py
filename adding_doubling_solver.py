...
def adding_doubling_solver(inputs):
    ...
    rdif_b = np.zeros(shape=[nbr_wvl,nbr_lyr+1])   #layer reflectivity to diffuse radiation from below
    tdir = np.zeros(shape=[nbr_wvl,nbr_lyr+1])   #layer transmission to direct radiation (solar beam + diffuse)
    tdif_a = np.zeros(shape=[nbr_wvl,nbr_lyr+1])   #layer transmission to diffuse radiation from above
    tdif_b = np.zeros(shape=[nbr_wvl,nbr_lyr+1])   #layer transmission to diffuse radiation from below
    trnlay = np.zeros(shape=[nbr_wvl,nbr_lyr+1])   #solar beam transm for layer (direct beam only)
    ...
    for lyr in np.arange(0,nbr_lyr,1):   # loop through layers
        ...
        for ng in np.arange(0,len(gauspt),1):     #gaussian angles (radians)
            ...
            trn = np.maximum(np.full((nbr_wvl,), exp_min), np.exp(-ts/mu))   # transmission
        ...
        if lyr == lyrfrsnl:
            ...
            for wl in np.arange(0,nbr_wvl,1): 
                ...
                if np.arccos(mu_not) < critical_angle[wl]:
                    ...
                    T2 = 2 * mu0[wl] / (nr[wl] * mu0[wl] + mu0n[wl])          #transmission amplitude factor for parallel polarization
                ...
                tdir[wl, lyr] = Tf_dir_a * tdir[wl, lyr] + Tf_dir_a*rdir[wl, lyr] * Rf_dif_b*rintfc*tdif_a[wl, lyr]   
                ...
                rdif_b[wl, lyr] = rdif_b[wl, lyr] + tdif_b[wl, lyr] * Rf_dif_b * rintfc * tdif_a[wl, lyr] 
        ...
        trndir[:,lyr+1] = trndir[:,lyr]*trnlay[:,lyr]  # solar beam transmission from top
        ...
        trntdr[:,lyr+1] = trndir[:,lyr]*tdir[:,lyr] + (tdndif + tdrrdir*rdndif[:,lyr])*refkm1*tdif_a[:,lyr] 
        
        # Eq. B4  Briegleb and Light 2007
        rdndif[:,lyr+1] = rdif_b[:,lyr] + (tdif_b[:,lyr]*rdndif[:,lyr]*refkm1*tdif_a[:,lyr])    #reflectivity to diffuse radiation for layers above
        trndif[:,lyr+1] = trndif[:,lyr]*refkm1*tdif_a[:,lyr]   #diffuse transmission to diffuse beam for layers above
    ...
    for lyr in np.arange(nbr_lyr-1,-1,-1):  # starts at the bottom and works its way up to the top layer
        ...
        rupdir[:,lyr] = rdir[:,lyr] + (trnlay[:,lyr] * rupdir[:,lyr+1] + (tdir[:,lyr]-trnlay[:,lyr])* rupdif[:,lyr+1])*refkp1*tdif_b[:,lyr] 
    ...
    for lyr in np.arange(0,nbr_lyr+1,1):
        ...
        fdirup[:,lyr] = (trndir[:,lyr]*rupdir[:,lyr] + (trntdr[:,lyr]-trndir[:,lyr]) * rupdif[:,lyr])*refk 
        ...
        fdirdn[:,lyr] = trndir[:,lyr] + (trntdr[:,lyr]- trndir[:,lyr] + trndir[:,lyr] * rupdir[:,lyr] * rdndif[:,lyr])*refk 
        ...
        # dfdir = fdirdn - fdirup
        dfdir[:,lyr] = trndir[:,lyr] + (trntdr[:,lyr]-trndir[:,lyr]) * (1 - rupdif[:,lyr]) * refk - trndir[:,lyr]*rupdir[:,lyr]\
            * (1 - rdndif[:,lyr]) * refk 
