...
for lwc_t in lwc_type:
    start = time.time()
    paramlist = list(
        set((itertools.product(depths, densities, grain_sizes, sza_list, lwcs, lwc_t)))
    )
    if __name__ == "__main__":
        nb_cores = 100
        pool = mp.Pool(nb_cores)
        ...
        df.columns = [str(i) for i in paramlist]
        df.to_feather(
            f"{path_to_save_files}/061024_lut_{lwc_t[0]}_lwc.feather",
            compression="zstd",
        )
        print("time for 1 lut: {}".format(time.time() - start))
