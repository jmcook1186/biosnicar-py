...
def call_SNICAR(z, density, grain_size, sza, lwc, alg, bc, dust):
    ...
    Inputs.rwater = [0] * len(
        Inputs.dz
    )  # int(Inputs.grain_rds[1]*1.055)]  # radius of optional liquid water coating
...
def bsnicarpy_wrapper(call_SNICAR, args):
    call_SNICAR(*args)
...
bcs = list(np.concatenate([np.arange(0, 900, 100), np.arange(900, 2100, 160)]))  # 17
...
paramtuples = list(
    set(
        (
            itertools.product(
                depths, densities, grain_sizes[0], sza_list, lwcs, algs, bcs, dusts
            )
        )
    )
)

paramlist = list(map(list, paramtuples))
...
for i in range(nb_runs):
    bsnicarpy_wrapper(call_SNICAR, paramlist[i])

end_time = time.time()
end_local_time = time.ctime(end_time)
processing_time = (end_time - start_time) / 60
print(f"--- Processing time ({nb_runs} runs): %s minutes ---" % processing_time)
