def toon_solver(Inputs):
    ...
    S_sfc = (
        R_sfc
        * mu_not
        * np.exp(-(tau_clm[nbr_lyr - 1, :] + tau_star[nbr_lyr - 1, :]) / mu_not)
        * np.pi
        * Fs
    )
    ...
    for i in np.arange(0, nbr_lyr, 1):
        ...
        if np.sum(Fs) > 0.0:
            ...
            C_pls_btm[i, :] = (
                ...
                * np.pi
                * Fs
                * np.exp(-(tau_clm[i, :] + tau_star[i, :]) / mu_not)
                * (
    ...
    AS[2 * nbr_lyr - 1, :] = np.nan_to_num(
        A[2 * nbr_lyr - 1, :] / B[2 * nbr_lyr - 1, :]
    )
    DS[2 * nbr_lyr - 1, :] = np.nan_to_num(
    ...
    BBAVIS = sum(flx_slr[0:vis_max_idx] * albedo[0:vis_max_idx]) / sum(
        flx_slr[0:vis_max_idx]
    )

    BBANIR = sum(
        flx_slr[vis_max_idx:nir_max_idx] * albedo[vis_max_idx:nir_max_idx]
    ) / sum(flx_slr[vis_max_idx:nir_max_idx])
