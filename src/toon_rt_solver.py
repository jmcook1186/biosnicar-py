...
def toon_solver(tau, ssa, g, L_snw, ice, illumination, rtm_cfg, model_cfg):
    ...
    S_sfc = (
        ice.sfc
        * illumination.mu_not
        * np.exp(-(tau_clm[ice.nbr_lyr - 1, :] + tau_star[ice.nbr_lyr - 1, :]) / illumination.mu_not)
        * np.pi
        * illumination.Fs
    )
    ...
    for i in np.arange(0, ice.nbr_lyr, 1):
        ...
        # (Toon et al. eq 50)
        direct[i, :] = (
            illumination.mu_not * np.pi * illumination.Fs * np.exp(-(tau_clm[i, :] + tau_star[i, :]) / illumination.mu_not)
        )
    ...
    albedo = F_top_pls / ((illumination.mu_not * np.pi * illumination.Fs) + illumination.Fd)

    # Net flux at upper model boundary
    F_top_net[0, :] = F_top_pls - ((illumination.mu_not * np.pi * illumination.Fs) + illumination.Fd)
    ...
    # bulk_reflected)
    energy_sum = (illumination.mu_not * np.pi * illumination.Fs) + illumination.Fd - (sum(F_abs) + F_btm_net + F_top_pls)
    ...
    total_insolation = np.sum((illumination.mu_not * np.pi * illumination.Fs) + illumination.Fd)
