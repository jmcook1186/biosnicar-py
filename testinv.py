...
def call_SNICAR(z, density, layer_types, grain_size, sza):
...
######################################
    ...
    inputs.dir_base = '/work/BioSNICAR_GO_PY/'
    ...
    inputs.grain_rds = [grain_size,grain_size] # effective grain radius of snow/bubbly ice (becomes bubble rds when layer_type==1)
    ...
    inputs.FILE_snw_alg  = 'dust_greenland_Cook_HIGH_20190911.nc' # Snow Algae (spherical, C nivalis) (Cook et al. 2017)
    inputs.FILE_glacier_algae = 'dust_greenland_Cook_HIGH_20190911.nc' # glacier algae in cells/ml or ppb depending on GA_units (Cook et al. 2020)
...
if __name__ == '__main__':
    ...
    res=pool.starmap(call_SNICAR,paramlist)
    pool.close()  
    pool.join() 
    df = pd.DataFrame.from_records(res)
    df = df.transpose()
    df.columns=paramlist
    df.to_csv('/work/test.csv',index=False)
